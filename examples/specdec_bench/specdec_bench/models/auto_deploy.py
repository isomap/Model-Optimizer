...
class AutoDeployModel(Model):
    def __init__(self, model_path, max_concurrent_requests, sampling_kwargs, **kwargs):
        self.model = create_auto_deploy_model(
            model_path, max_concurrent_requests, kwargs
        )
...
def check_sampling_config(
    sampling_config: Dict[str, Any], max_length: int, end_id: int
):
