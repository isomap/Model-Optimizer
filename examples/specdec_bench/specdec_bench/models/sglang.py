...
class SGLANGModel(Model):
        ...
        **kwargs,
        ...
        if speculative_algorithm is not None:
            ...
            self.model = sgl.Engine(
                ...
                speculative_num_draft_tokens=kwargs.get(
                    "speculative_num_draft_tokens", 4
                ),
                speculative_draft_model_path=kwargs.get("draft_model_dir", None),
                torch_compile_max_bs=max_concurrent_requests,
                max_running_requests=max_concurrent_requests,
                attention_backend=kwargs.get("attention_backend", None),
    ...
    async def run(self, prompt_ids, max_length, end_id, request_id, turn_id):
        ...
        reformatted_output_ids = [
            [] for _ in range(self.sampling_config.get("beam_width", 1))
        ]
