...
try:
    from datasets import load_dataset, concatenate_datasets  # type: ignore[attr-defined]
    import tiktoken
    import numpy as np
    import pandas as pd
...
DATASETS_AND_LOADERS_FUNCTIONS = {
    ...
    BenchmarkDataset.CNN_DAILYMAIL.value: lambda dataset_name,
    config_name: load_dataset(dataset_name, config_name, split="test"),
    ...
    BenchmarkDataset.LIVECODEBENCH.value: lambda dataset_name,
    ...
    BenchmarkDataset.CODE_CONTESTS.value: lambda dataset_name,
    config_name: load_dataset(
    ...
    BenchmarkDataset.CHATRAG_BENCH.value: lambda dataset_name,
    config_names: concatenate_datasets(
        [
            load_dataset(
                dataset_name,
                config_name,
    ...
    BenchmarkDataset.ADALEVAL_STACKSELECT.value: lambda dataset_name,
    config_name: load_dataset("json", data_files={"test": config_name}, split="test"),
    BenchmarkDataset.ADALEVAL_TEXTSORT.value: lambda dataset_name,
    config_name: load_dataset("json", data_files={"test": config_name}, split="test"),
    BenchmarkDataset.ROLEBENCH.value: lambda dataset_name, config_name: pd.read_json(
        config_name, lines=True
    ),
    BenchmarkDataset.ROLEBENCH_ROLES.value: lambda dataset_name,
    config_name: load_dataset("json", data_files={"test": config_name}, split="test"),
...
class SPEEDBench(Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        if not_installed:
            raise ImportError(
                "datasets, tiktoken, and numpy packages are required to use SPEED-Bench. Please install it with `pip install -r requirements.txt`"
            )
        self.data: List[Request] = []
        self.num_samples = num_samples
        self.external_datasets: dict[str, Any] = dict()
        self._config_name = config_name
        self._resolved_dataset = None
        self._preprocess(config_name, _prepare_mode=_prepare_mode)

    def _get_external_dataset(
        self, dataset_name: str, config_name: str | list[str] = "default"
    ):
        full_name = f"{dataset_name}_{config_name}"
        if full_name not in self.external_datasets:
            self.external_datasets[full_name] = DATASETS_AND_LOADERS_FUNCTIONS[
                dataset_name
...
"""
        ...
        for i, answer in enumerate(answers):
            ...
            answer_to_add_tokens = len(
                encoder.encode(answer_to_add, disallowed_special=())
            )
...
"""
    ...
    def _pad_or_truncate_prompt(
        ...
        if current_num_tokens > target_num_tokens:
            ...
            num_padding_repeats = (tokens_needed + len(padding_tokens) - 1) // len(
                padding_tokens
            )
...
her fear and anger)."""
        ...
        other_character_profiles_str = "\n\n".join(
            [
                ...
                for character_name, character_profile in external_dataset[
                    "character_profiles"
                ][0].items()
    ...
    @staticmethod
    def _generate_mmlu_pro_prompt(
        external_dataset: "Dataset", subject: str
    ) -> list[Any]:
        def get_question_and_options(question, options):
            options = [(chr(ord("A") + i), a) for i, a in enumerate(options)]
            options_str = "\n".join(
                [f"({letter}) {option}" for letter, option in options]
            )
    ...
    def _get_num_tokens_from_config(speed_config: config_type | str) -> int:
        ...
        if match:
            ...
            raise ValueError(
                f"Could not determine num_tokens from speed_config: {speed_config}"
            )
    ...
    def _fetch_all_turns_data(
        ...
        if BenchmarkDataset.BAMBOO.value in example["source"]:
            ...
            example["turns"] = [
                self._generate_bamboo_prompt(external_dataset, num_tokens)
            ]
            ...
            article = external_dataset[external_dataset["id"] == src_id][
                "article"
            ].iloc[0]
            example["turns"] = [
                example["turns"][0]
            ...
            if "qualitative" in speed_config:
                ...
                src_id = example["src_id"]
                example["turns"] = [
                    external_dataset[external_dataset["id"] == src_id]["question"].iloc[
                        0
                    ]
                ]
                ...
                hle_train["tokens"] = hle_train["demonstration"].apply(
                    lambda e: tiktoken.get_encoding("o200k_base").encode(
                        e, disallowed_special=()
                    )
                )
                src_id = example["src_id"]
            ...
            example["turns"] = [
                entry["user"] for entry in external_dataset_example["history"][0]
            ]
            ...
            src_id = example["src_id"].rsplit("_", 1)[1]
            external_dataset_example = external_dataset.select([int(src_id)])
            example["turns"] = self._generate_chatrag_bench_prompt(
                external_dataset_example
            )
            ...
            example["turns"] = [
                self._pad_or_truncate_prompt(
                    self._generate_textsort_prompt(
                        external_dataset_example["prompt"][0]
                    ),
                    num_tokens,
                )
            ]
            ...
            src_ids = [
                int(match) for match in re.findall(r"_(\d+)", example["src_id"])
            ][: len(example["turns"])]
    ...
    def _load_dataset(
        self, config_name_or_dataset_path: config_type | str
    ) -> "Dataset":
        ...
        if config_name_or_dataset_path in get_args(config_type):
            dataset = load_dataset(
                "nvidia/SPEED-Bench", config_name_or_dataset_path, split="test"
            )
        else:
            ...
            if config_name_or_dataset_path_path.is_dir():
                data_files = {
                    "test": [
                        str(path)
                        for path in config_name_or_dataset_path_path.rglob("*.parquet")
                    ]
                }
            else:
    ...
    def _resolve_external_data(
        ...
        """
        return dataset.map(  # type: ignore[attr-defined]
            self._fetch_all_turns_data, fn_kwargs={"speed_config": speed_config}
        )

    def _preprocess(  # type: ignore[override]
        self,
        ...
        *,
        ...
        self._resolved_dataset = dataset  # type: ignore[assignment]
        self.data = [
            Request(
                system_prompt=None,
                turns=example["turns"],
    ...
    def prepare_data(  # type: ignore[override]
        ...
        instance._resolved_dataset.to_parquet(output_path)  # type: ignore[attr-defined]
        return output_path
