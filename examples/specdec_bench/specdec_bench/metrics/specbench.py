...
class SpecBench(AcceptanceRate):
    ...
    def process_final(self, text_outputs):
        ...
        for request in self.requests:
            per_category[request.category].append(
                self.out["Request_AR"][request.question_id]
            )
    ...
    def _format_write_output(self, outputs):
        with open(
            os.path.join(self.directory, "specbench_responses.jsonl"), "w"
        ) as outfile:
    ...
    def _pretty_print_results(self):
        ...
        table.add_section()
        table.add_row(
            "[bold]Overall Average[/bold]", f"[bold]{self.out['Average_AR']:.4f}[/bold]"
        )
    ...
    def _dump_results(self):
        with open(
            os.path.join(self.directory, "specbench_results.json"), "w"
        ) as outfile:
