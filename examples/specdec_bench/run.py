...
from specdec_bench import models, runners, datasets, metrics
...
async def run_loop(
    ...
    tasks = [
        process_single_request(request, i) for i, request in enumerate(dataset.data)
    ]
    if show_progress:
    ...
    for i, result in enumerate(text_outputs):
        if isinstance(result, Exception):
            print(
                f"Error processing request {i}/{dataset.data[i].question_id}: {result}"
            )
            raise result
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--draft_length", type=int, required=False, default=3, help="Draft length"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--aa_timing", action="store_true", help="Enable AA timing metric"
    )
    ...
    if args.runtime_params is not None:
        with open(args.runtime_params, "r") as f:
