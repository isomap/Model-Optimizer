...
# limitations under the License.
#
# Extracted from vLLM (vllm/v1/attention/ops/triton_prefill_attention.py).
# vLLM adapted from SGLang, which adapted from LightLLM (ModelTC/lightllm).
# Changes for ModelOpt: removed vLLM-specific imports (platforms, triton_utils,
# math_utils), made standalone using torch and triton directly.

"""Memory-efficient attention for prefill.

Supports variable sequence lengths via b_start_loc / b_seq_len.
"""
...
# 1 / log(2), used to rescale softmax scale for triton exp2
...
def _fwd_kernel(
    ...
    for start_n in range(start_n_limit, end_n_limit, BLOCK_N):
        ...
        # Bidirectional sliding window: apply to qk via tl.where (avoids mask updates in constexpr branches)
        if SLIDING_WINDOW_Q > 0:
            qk = tl.where(pos_q - pos_k <= SLIDING_WINDOW_Q, qk, -1.0e8)
        if SLIDING_WINDOW_K > 0:
            qk = tl.where(pos_k - pos_q <= SLIDING_WINDOW_K, qk, -1.0e8)
...
def get_block_size(dtype: torch.dtype) -> int:
    ...
    if torch.cuda.is_available():
        ...
        if capability[0] >= 8:  # Ampere (SM 80) and above
            return 128
    return 64
...
def context_attention_fwd(
    ...
    """Memory-efficient prefill attention forward pass.
    ...
    Args:
        ...
        b_start_loc: Start index per batch [batch_size].
        b_seq_len: Sequence length per batch [batch_size].
        max_input_len: Maximum sequence length in the batch.
        is_causal: If True, apply causal masking.
        softmax_scale: Scale for attention scores (default: 1/sqrt(head_dim)).
        sliding_window_q: Sliding window size for query (None or 0 = disabled).
        sliding_window_k: Sliding window size for key (None or 0 = disabled).
    """
    ...
    # Triton dot requires M, N, K >= 16; pad BLOCK_DMODEL when head_dim < 16 (kernel masks with Lk)
    BLOCK_DMODEL = max(triton.next_power_of_2(Lk), 16)
