...
# def safe_safetensors_by_layer_index(
...
#     for layer_index, layer_state_dicts in enumerate(layer_state_dicts):
...
#         weight_map = {}
#         layer_total_size = 0
#         for key, val in layer_state_dicts.items():
#             tensor_size = val.numel() * val.element_size()
...
#             )
#         save_file(
#             layer_state_dicts, save_directory + "/" + ckpt_filename, metadata={"format": "pt"}
#         )
#
...
#
#     if torch.distributed.get_rank() == 0:
#         global_count = total_layers
#         safetensor_index = {
#             "metadata": {"total_size": 0},
#             "weight_map": {},
#         }
#         for layer_index in range(total_layers):
#             meta_filename = name_template.format(layer_index, total_layers)
#             with open(save_directory + "/" + meta_filename) as f:
#                 shard = json.load(f)
#             safetensor_index["metadata"]["total_size"] += shard["metadata"]["total_size"]
#             safetensor_index["weight_map"].update(shard["weight_map"])
